...
public:
  ...
  Device(const string & name, const string & pnode, const string & nnode, double val):
	  _name(name), _pnode_name(pnode), _nnode_name(nnode), _value(val) { }

  /// another constructor
  Device(string & line);

  //Device(const string & name, const string & pnode_name, const string & nnode_name, double val):
	 // _name(name), _pnode_name(pnode_name), _nnode_name(nnode_name), _value(val) { }

  void setPnodeName(const string &n) { _pnode_name = n;    }
  void setNnodeName(const string &n) { _nnode_name = n;    }

  string pnodeName() const			 { return _pnode_name; }
  string nnodeName() const			 { return _nnode_name; }

  /// auxilary output function
  virtual void output();
